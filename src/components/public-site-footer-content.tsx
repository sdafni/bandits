...
export function PublicSiteFooterContent({ showTrustLayer = true }: { showTrustLayer?: boolean }) {
  const { locale, t } = useLocale();

  const trustItems = [
    { title: t("footer.gdprTitle"), description: t("footer.gdprBody"), icon: ShieldCheck },
    { title: t("footer.secureTitle"), description: t("footer.secureBody"), icon: FileLock2 },
    { title: t("footer.supportTitle"), description: t("footer.supportBody"), icon: LifeBuoy },
  ];
        ...
        <div className="relative z-[1] grid gap-8 lg:grid-cols-[1.1fr_0.9fr]">
          ...
          <div className="grid gap-8 sm:grid-cols-3">
            <div className="space-y-3" id="support">
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#8b6b17]">{t("footer.legal")}</p>
              <div className="space-y-2 text-sm text-slate-700">
                <Link className="block transition hover:text-[#0f2343]" href={withLocalePath(locale, "/privacy")}>
                  {t("footer.privacy")}
                </Link>
                <Link className="block transition hover:text-[#0f2343]" href={withLocalePath(locale, "/terms")}>
                  {t("footer.terms")}
                </Link>
              </div>
            </div>
            ...
            <div className="space-y-3">
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#8b6b17]">{t("footer.dataProtection")}</p>
              <p className="text-sm leading-7 text-slate-700">{t("footer.dataProtectionBody")}</p>
            </div>

            <div className="space-y-3">
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#8b6b17]">{t("footer.contact")}</p>
              <p className="text-sm leading-7 text-slate-700">{t("footer.contactBody")}</p>
              <PublicSupportForm />
            </div>
          </div>
        </div>
