      ...
      <nav aria-label={t("appNav.landlordNavLabel")} className={cn("flex flex-wrap gap-2", className)}>
        {items.map(({ key, href, label, icon: Icon }) => (
          <Link
            ...
            key={key}
          >
            <Icon className={cn("h-4 w-4 shrink-0", active === key && "text-white")} aria-hidden />
            {label}
          </Link>
        ))}
      </nav>
            ...
            >
              <span
                className={cn(
                  ...
                  isActive ? "landlord-bottom-nav__pill--active text-white" : "",
                  "sm:inline-flex sm:min-h-0 sm:flex-row sm:gap-2 sm:bg-transparent sm:p-0 sm:text-inherit",
                )}
              >
                <Icon
                  className={cn(
                    "h-[1.125rem] w-[1.125rem] shrink-0 sm:h-4 sm:w-4",
                    isActive && "text-white",
                  )}
                  aria-hidden
                />
